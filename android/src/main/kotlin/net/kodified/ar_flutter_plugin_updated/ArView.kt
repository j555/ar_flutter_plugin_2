...
class ArView(
    ...
    init {
        trace("Hardware Boot: Initializing ARCore Engine...")
    ...
    private fun setupSceneViewListeners() {
        sceneView.onSessionUpdated = { _, frame ->
            if (!isDestroyed) {
                ...
                // 🎯 FIX: Release heavy hardware handles every frame to prevent Buffer Lost errors
                try {
    ...
    private fun broadcastHardwareTelemetry(frame: Frame) {
        ...
        hits.firstOrNull { it.trackable is Plane }?.let { hit ->
            ...
            // Precision Math using raw native floats
            val hp = hit.hitPose
            ...
            if (System.currentTimeMillis() - lastDebugTime > 3000) {
                lastDebugTime = System.currentTimeMillis()
                trace("Telemetry Sync | Dist: ${String.format("%.3fm", dist)} | Tilt: ${String.format("%.1f°", tilt)}")
            }
    ...
    private fun handleCaptureBundle(result: MethodChannel.Result) {
        ...
        val camera = frame.camera
        trace("Snapshot Started: Locking hardware matrices...")

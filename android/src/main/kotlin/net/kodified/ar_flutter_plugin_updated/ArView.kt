...
class ArView(
    ...
    private fun broadcastUnifiedUpdate(frame: Frame) {
        ...
        val images = frame.getUpdatedTrackables(AugmentedImage::class.java).map { mapOf("name" to it.name, "index" to it.index, "tracking" to it.trackingState.name) }
        if (images.isNotEmpty()) packet["augmentedImages"] = images
        isBridgeBusy = true
        activity.runOnUiThread { if (!isDestroyed) sessionChannel.invokeMethod("onUnifiedUpdate", packet); isBridgeBusy = false }
    }
        ...
        PixelCopy.request(sceneView, bitmap, { copyResult ->
            if (copyResult == PixelCopy.SUCCESS) {
                mainScope.launch(Dispatchers.IO) {
                    ...
                    try { frame.acquireDepthImage16Bits().use { img -> val buffer = img.planes[0].buffer; val bytes = ByteArray(buffer.remaining()); buffer.get(bytes); data["depthMap"] = bytes } } catch (e: Exception) {}
    ...
    private fun handleRemoveNode(call: MethodCall, result: MethodChannel.Result) {
        ...
        nodesMap[name]?.let { node ->
            ...
            anchorNodesMap.values.find { it.childNodes.contains(node) }?.let { anchor ->
                if (anchor.childNodes.isEmpty()) { sceneView.removeChildNode(anchor); anchor.anchor?.detach(); anchorNodesMap.entries.removeIf { it.value == anchor } }
            }
            result.success(name) 
    ...
    private fun handleRemoveAnchor(name: String?, result: MethodChannel.Result) {
        anchorNodesMap[name]?.let { sceneView.removeChildNode(it); it.anchor?.detach(); anchorNodesMap.remove(name); result.success(null) } ?: result.error("ERR", "Anchor missing", null)
    }
    ...
    private fun updatePointCloud(frame: Frame) {
        ...
        try {
            if (pointCloud.timestamp != lastPointCloudTimestamp) {
                ...
                pointCloudNodes.removeAll { node -> if (!currentIds.contains(node.id)) { sceneView.removeChildNode(node); pointCloudNodePool.add(node); true } else false }
    ...
    private suspend fun buildModelNode(nodeData: Map<String, Any>): ModelNode? {
        ...
        return try { sceneView.modelLoader.loadModelInstance(uri)?.let { inst -> ModelNode(inst).apply { name = nodeData["name"] as? String; (nodeData["transformation"] as? ArrayList<Double>)?.let { val s = it[0].toFloat(); scale = Scale(s, s, s) } } } } catch (e: Exception) { null }
    }

    private fun handleAddAnchor(call: MethodCall, result: MethodChannel.Result) {
        (call.argument<ArrayList<Double>>("transformation"))?.let { t ->
            val (p, r) = deserializeMatrix4(t)
            sceneView.session?.createAnchor(Pose(floatArrayOf(p.x, p.y, p.z), floatArrayOf(r.x, r.y, r.z, r.w)))?.let { val node = AnchorNode(sceneView.engine, it); sceneView.addChildNode(node); anchorNodesMap[call.argument<String>("name") ?: "anchor"] = node; result.success(true) } ?: result.success(false)
    ...
    private fun handleUploadAnchor(call: MethodCall, result: MethodChannel.Result) { anchorNodesMap[call.argument<String>("name")]?.let { node -> val cloud = CloudAnchorNode(sceneView.engine, node.anchor!!); cloud.host(sceneView.session!!) { id, state -> if (state == Anchor.CloudAnchorState.SUCCESS) result.success(id) else result.error("ERR", state.name, null) }; sceneView.addChildNode(cloud) } ?: result.error("ERR", "Missing anchor", null) }
    private fun handleDownloadAnchor(call: MethodCall, result: MethodChannel.Result) { (call.argument<String>("cloudanchorid"))?.let { id -> CloudAnchorNode.resolve(sceneView.engine, sceneView.session!!, id) { state, node -> if (!state.isError && node != null) { sceneView.addChildNode(node); result.success(true) } else result.error("ERR", state.name, null) } } ?: result.error("ERR", "No ID", null) }
    private fun handleGetCameraPose(result: MethodChannel.Result) { val pose = FloatArray(16); currentArFrame?.camera?.displayOrientedPose?.toMatrix(pose, 0); result.success(pose.map { it.toDouble() }) }
    private fun handleGetProjectionMatrix(result: MethodChannel.Result) { val proj = FloatArray(16); currentArFrame?.camera?.getProjectionMatrix(proj, 0, 0.01f, 100f); result.success(proj.map { it.toDouble() }) }
    private fun handleGetImageIntrinsics(result: MethodChannel.Result) { currentArFrame?.camera?.imageIntrinsics?.let { i -> result.success(mapOf("fx" to i.focalLength[0].toDouble(), "fy" to i.focalLength[1].toDouble(), "cx" to i.principalPoint[0].toDouble(), "cy" to i.principalPoint[1].toDouble(), "width" to i.imageDimensions[0].toDouble(), "height" to i.imageDimensions[1].toDouble())) } ?: result.error("ERR", "No intrinsics", null) }
    ...
    private fun serializePlane(plane: Plane): Map<String, Any> { val matrix = FloatArray(16); plane.centerPose.toMatrix(matrix, 0); return mapOf("identifier" to plane.hashCode().toString(), "centerPose" to matrix.map { it.toDouble() }, "extent" to listOf(plane.extentX.toDouble(), plane.extentZ.toDouble())) }
    private fun processPendingHits(frame: Frame) { while (!pendingHitTests.isEmpty()) { val req = pendingHitTests.poll() ?: break; frame.hitTest(req.x, req.y).firstOrNull { it.trackable is Plane || it.trackable is com.google.ar.core.Point }?.let { hit -> val node = AnchorNode(sceneView.engine, hit.createAnchor()); sceneView.addChildNode(node); mainScope.launch { buildModelNode(req.nodeData)?.let { m -> node.addChildNode(m); req.result.success(true) } ?: req.result.success(false) } } } }
    private fun handleAddNodeToPlaneAnchor(call: MethodCall, result: MethodChannel.Result) { val data = call.arguments as? Map<String, Any>; val nData = data?.get("node") as? Map<String, Any>; val aData = data?.get("anchor") as? Map<String, Any>; anchorNodesMap[aData?.get("name") as? String]?.let { anchorNode -> mainScope.launch { buildModelNode(nData!!)?.let { n -> anchorNode.addChildNode(n); n.name?.let { nodesMap[it] = n }; result.success(true) } ?: result.success(false) } } ?: result.success(false) }
    ...
    private fun getPointCloudModelInstance(): ModelInstance? { if (pointCloudModelInstances.isEmpty()) { pointCloudModelInstances = sceneView.modelLoader.createInstancedModel("models/point_cloud.glb", maxPoints).toMutableList() }; return pointCloudModelInstances.removeLastOrNull() }
    private fun handleShowWorldOrigin(show: Boolean) { if (show && worldOriginNode == null) { val loader = MaterialLoader(sceneView.engine, viewContext); worldOriginNode = Node(sceneView.engine).apply { addChildNode(CylinderNode(sceneView.engine, radius = 0.005f, height = 0.1f, materialInstance = loader.createColorInstance(io.github.sceneview.math.Color(1f, 0f, 0f, 1f)))) }; sceneView.addChildNode(worldOriginNode!!) } else if (!show) { worldOriginNode?.let { sceneView.removeChildNode(it) }; worldOriginNode = null } }
    private fun handleTransformNode(call: MethodCall, result: MethodChannel.Result) { val name = call.argument<String>("name"); val t = call.argument<ArrayList<Double>>("transformation"); nodesMap[name]?.apply { transform(dev.romainguy.kotlin.math.Mat4.of(*t!!.map { it.toFloat() }.toFloatArray())); result.success(null) } ?: result.error("ERR", "Not found", null) }
    override fun onStateChanged(source: LifecycleOwner, event: Lifecycle.Event) { if (!isDestroyed) { if (event == Lifecycle.Event.ON_DESTROY) dispose() else lifecycleRegistry.handleLifecycleEvent(event) } }
    override fun getView(): View = rootLayout
    override fun dispose() { if (isDestroyed) return; isDestroyed = true; mainScope.cancel(); activity.runOnUiThread { activityLifecycle.removeObserver(this@ArView); sceneView.onSessionUpdated = null; sceneView.session?.pause(); lifecycleRegistry.handleLifecycleEvent(Lifecycle.Event.ON_DESTROY); sessionChannel.setMethodCallHandler(null); objectChannel.setMethodCallHandler(null); anchorChannel.setMethodCallHandler(null); rootLayout.removeAllViews() } }
}